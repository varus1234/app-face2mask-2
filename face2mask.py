...
def face2mask(imgs_face):
    ...
    file_name = '/tmp/120_net_G.pth'
    urllib.request.urlretrieve(url, file_name)
    
    norm_layer = functools.partial(nn.BatchNorm2d, affine=True, track_running_stats=True)
    net = UnetGenerator(3, 3, 8, 64, norm_layer=norm_layer, use_dropout=True)
    init_weights(net, 'normal', 0.02)
    net.load_state_dict(torch.load('/tmp/120_net_G.pth', map_location=torch.device('cpu')))
